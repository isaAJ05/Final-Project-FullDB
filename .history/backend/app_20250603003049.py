    ...
    try:
       # CREATE DATABASE
        ...
        if query.lower().startswith("drop database"):
            ...
            if not match:
                return jsonify({'error': 'Sintaxis inválida para DROP DATABASE'}), 400
            db_name = match.group(1)
            db_path = os.path.join(DATA_DIR, db_name)
            if not os.path.exists(db_path):
                return jsonify({'error': f'La base de datos {db_name} no existe'}), 400
            shutil.rmtree(db_path)
            return jsonify({'message': f'Base de datos {db_name} eliminada'})

        # RENAME DATABASE old_name TO new_name
        if query.lower().startswith("rename database"):
            match = re.match(r"rename database (\w+) to (\w+)", query, re.IGNORECASE)
            if not match:
                return jsonify({'error': 'Sintaxis inválida para RENAME DATABASE'}), 400
            old_name, new_name = match.groups()
            old_path = os.path.join(DATA_DIR, old_name)
            new_path = os.path.join(DATA_DIR, new_name)
            if not os.path.exists(old_path):
                return jsonify({'error': f'La base de datos {old_name} no existe'}), 400
            if os.path.exists(new_path):
                return jsonify({'error': f'La base de datos {new_name} ya existe'}), 400
            os.rename(old_path, new_path)
            return jsonify({'message': f'Base de datos renombrada de {old_name} a {new_name}'})
