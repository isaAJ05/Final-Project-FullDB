    ...
    try:
       # CREATE DATABASE
        ...
        if query.lower().startswith("drop table"):
            ...
            if not db:
                return jsonify({'error': 'Debes especificar la base de datos: DROP TABLE db.tabla'}), 400
        ...
        if query.lower().startswith("rename table"):
            ...
            if not db or not db_new or db != db_new:
                return jsonify({'error': 'Ambas tablas deben estar en la misma base de datos'}), 400
