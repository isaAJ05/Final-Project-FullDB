...
def save_table(db, table, data):
    """Guarda la tabla como archivo JSON en disco."""
...
def load_table(db, table):
    """Carga la tabla desde archivo JSON. Si no existe, retorna lista vacía."""
    try:
...
def backup_table(db, table, data):
    """Crea un respaldo de la tabla antes de modificarla."""
...
# ==========================
# ENDPOINT PRINCIPAL: EJECUCIÓN DE SQL
# ==========================

@app.route('/execute', methods=['POST'])
def execute_sql():
    """
    Recibe una consulta SQL, la valida con sqlparse y ejecuta la lógica correspondiente.
    Soporta: CREATE DATABASE, CREATE TABLE, DROP TABLE, RENAME TABLE, INSERT, UPDATE, DELETE, SELECT.
    """
    data = request.json
    ...
    try:
        ...
        stmt = parsed[0]
        # Validar el tipo de sentencia soportada
