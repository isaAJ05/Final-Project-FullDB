...
def executor(plan, stmt_type, query, data, stmt_info):
    ...
    if stmt_type == "SELECT":
        ...
        if stmt_info["joins"]:
            ...
            for row in joined:
                ...
                # Agregados (solo fila a fila, útil para COUNT/SUM sin GROUP BY)
        ...
        if stmt_info["group_by"]:
            group_col = stmt_info["group_by"][0].split(".")[-1]
            agg = stmt_info["aggregates"][0]
            agg_func = agg["func"]
    ...
    # SELECT avanzado: JOIN y GROUP BY usando stmt_info
    if stmt_type == "SELECT":
        if stmt_info["joins"]:
            # JOIN simple
            main_table = stmt_info["tables"][0]
            ...
            for row in joined:
                ...
                for agg in stmt_info["aggregates"]:
                    if agg["func"] == "SUM":
                        result_row[agg["alias"] or f"sum_{agg['col']}"] = float(row.get(agg["col"], 0))
                    elif agg["func"] == "COUNT":
                        result_row[agg["alias"] or f"count_{agg['col']}"] = 1
                result.append(result_row)
            return {"source": "executed", "rows": result}
