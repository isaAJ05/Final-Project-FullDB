    ...
    if stmt_type == "SELECT":
        ...
        for expr in stmt.args["expressions"]:
            if hasattr(expr, "alias_or_name"):
                selected_cols.append(expr.alias_or_name)
            else:
                selected_cols.append(str(expr))

        # Filtrar solo las columnas seleccionadas
        filtered = [
            {col: row.get(col) for col in selected_cols if col in row}
            for row in joined
        ]
        return {"source": "executed", "rows": filtered}
        # GROUP BY + SUM/COUNT
