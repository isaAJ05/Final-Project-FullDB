...
function App() {
  ...
  <div style={{
    ...
    <svg width="18" height="18" fill="none" stroke="currentColor" style={{marginRight: 4}}><path d="M3 9.5V5a2 2 0 012-2h10a2 2 0 012 2v4.5" strokeWidth="2"/><path d="M3 9.5l7 7 7-7" strokeWidth="2"/></svg>
    Localhost / {expandedDb ? expandedDb : "Databases"}
  </div>
  ...
  <h3 style={{
    ...
    <svg width="20" height="20" fill="none" stroke="currentColor" style={{marginRight: 4}}><rect x="3" y="5" width="14" height="10" rx="2" strokeWidth="2"/></svg>
    Bases de Datos
  </h3>
  {databases.length === 0 ? (
      ...
      <div key={db}>
        <button
          ...
          style={{
            ...
            background: expandedDb === db ? "#22263a" : undefined,
            display: "flex",
            alignItems: "center",
            gap: 6
          }}
            ...
            {tablesByDb[db] && tablesByDb[db].length > 0 ? (
              tablesByDb[db].map((table) => (
                <button
                  ...
                  onClick={() => setQuery(`SELECT * FROM ${db}.${table} LIMIT 10`)}
